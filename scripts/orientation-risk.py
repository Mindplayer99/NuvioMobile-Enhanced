...
CONTRACT = re.compile(r'requestedOrientation|SCREEN_ORIENTATION|PlayerOrientation|player_orientation|rememberedOrientation|orientationOverride|CompactPlayerTrackSheet|supportsPictureInPicture|resizeableActivity|android:screenOrientation|android:configChanges')
...
LIFECYCLE = re.compile(r'LaunchedEffect|DisposableEffect|onDispose|onStop|onPause|onResume|onConfigurationChanged|isInPictureInPictureMode|isInMultiWindowMode|sessionId|requestedOrientation|playbackSession|sourceId|contentId')
...
def classify(path, diff, canonical=''):
    ...
    if report['risk'] != 'hard':
        report['risk'] = 'low' if data or tests or not any(code(h['lines']) for h in changes) else 'soft'
